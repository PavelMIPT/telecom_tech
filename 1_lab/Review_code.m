...
rng(101);  % фиксирует состояние регистров - фиксируем матрицу случайных чисел
% TO-DO 1%
...
n = 1000;  % number of times we will record
Data_time = cell(n, 2);
...
% Data collection
for n = 10 : N
    average_legacy_time1 = 0;
    average_optimise_time1 = 0;
...
%% grafic illustration
grafic = plot(Datay', cell2mat(Datax(:, 1)));
hold on;
plot(Datay', cell2mat(Datax(:, 2)));
xlabel('число итераций');
ylabel('среднее время выполнения');
legend('legacy', 'optimise');

saveas(grafic, 'average_time(iteration).png');
%% Function discribing
