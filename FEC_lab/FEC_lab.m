...
for ii = 1:length(snr)
    for counter = 1:numframes
        ...
        % dencodedData = BFDecoder(demap, maxIterationsBF, H);
        % errStats(ii, counter, :) = ber_encode(encodedData, dencodedData);
        %========================================
...
grid on;
set(gca, 'Fontsize', 20);
% saveas(fig1, 'with_LDPC_coding.png')
...
%         encodedData = ldpcEncode(data,cfgLDPCEnc);
% %         decodedCodeWord = BFDecoder(encodedData.', maxnumiter, H);
% 
...
%         llr = transpose(demapping(noised_sig, constel));
% 
